...
- Turn the telemetry of one agent run into data journalism.
- Emit byte-for-byte deterministic HTML and SVG: no model, no network, no randomness.
- Frame a small sample (n=1) as a single result, without inflating it into a rate.
- Why this generator runs no model, and why that is the right call here.
- Read a `RunRecord` and derive traceable figures and four accessible charts from it.
...
The story it tells is data journalism about one run. A controller read one
...
narrates it, so the finding is reported as one run's result, not a rate.
...
- **Small-sample / n=1 framing.** The run is a single sample. The code carries
...
- That a "wow" finding can be reported **accurately**: the counterintuitive result
...
the guarantees are enforced. Key assertions:
...
builder would be counterproductive: it would invite invented or paraphrased numbers where the
whole point is that no number is invented.

Keeping the transform pure -- JSON in, HTML out, no model, no network,
byte-for-byte deterministic -- is what makes the guarantees testable and the
...
as a learning tool for turning agent telemetry into data journalism.
